...
m5.polr <- polr(factor(result) ~ buildUpPlaySpeed_ratio+buildUpPlayDribbling_ratio+
...
predictions = predict(m5.polr, newdata=m5[-train,], type='probs')
predict.value = ifelse(predictions[,1]>=predictions[,2] & predictions[,1]>=predictions[,3],
                       0, ifelse(predictions[,2]>=predictions[,3], 1, 2))
confusion_matrix = table(m5[-train,]$result, predict.value)
